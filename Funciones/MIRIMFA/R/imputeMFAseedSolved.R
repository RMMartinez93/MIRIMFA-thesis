#' Impute MFA with seed use problems solved
#'
#' This function is a version of missMDA's imputeMFA that does not crash when setting seeds.
#'
#' @param X A data frame with groups of continuous or categorical variables containing missing values.
#' @param group A vector indicating the number of variables in each group.
#' @param ncp An integer corresponding to the number of components used to predict the missing entries.
#' @param type A vector indicating the type of variables in each group; three possibilities:
#'   \itemize{
#'     \item "c" for continuous variables (the variables are centered).
#'     \item "s" for continuous variables (the variables are scaled to unit variance).
#'     \item "n" for categorical variables.
#'   }
#' @param method A string indicating the imputation method; "Regularized" by default or "EM".
#' @param row.w A numeric vector of row weights (by default, a vector of 1 for uniform row weights).
#' @param coeff.ridge A numeric value; by default 1 to perform the regularized `imputeMFA` algorithm.
#'   This is only used when `method="Regularized"`. Lower values reduce regularization, and higher values increase regularization.
#' @param threshold A numeric value for the threshold used to assess convergence.
#' @param ind.sup A vector indicating the indexes of the supplementary individuals (those not used in the imputation).
#' @param num.group.sup A vector indicating the group of variables that are supplementary (those not used in the imputation).
#' @param seed An integer value for the random seed. If `NULL` (default), missing values are initially imputed by the mean of each continuous variable and by the category proportion for categorical variables. A specific integer seed leads to random initialization.
#' @param maxiter An integer specifying the maximum number of iterations for the algorithm.
